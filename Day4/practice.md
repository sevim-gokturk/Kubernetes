...
- Create a `sevim-pv.yaml` file using the following content with the volume type of `hostPath` to build a `PersistentVolume` and explain fields.
...
- Create the PersistentVolume `sevim-pv-vol`.

```bash
kubectl apply -f sevim-pv.yaml
```
...
- Create a `sevim-pv-claim.yaml` file using the following content to create a `PersistentVolumeClaim` and explain fields.
...
- Create the PersistentVolumeClaim `sevim-pv-claim`.

```bash
kubectl apply -f sevim-pv-claim.yaml
```
...
- View information about the `PersistentVolumeClaim` and show that the `PersistentVolumeClaim` is bound to your PersistentVolume `sevim-pv-vol`.

```bash
kubectl get pvc sevim-pv-claim
```
...
- Create a `sevim-pod.yaml` file that uses your PersistentVolumeClaim as a volume using the following content.
...
spec:
  volumes:
    - name: sevim-pv-storage
      persistentVolumeClaim:
        claimName: sevim-pv-claim
  containers:
    - name: sevim-pv-container
      image: nginx
      ports:
        - containerPort: 80
          name: "http-server"
...
- Create the Pod `sevim-pod`.

```bash
kubectl apply -f sevim-pod.yaml
```

- Verify that the Pod is running.

```bash
kubectl get pod sevim-pod
```

- Open a shell to the container running in your Pod.

```bash
kubectl exec -it sevim-pod -- /bin/bash
```
...
- Expose the sevim-pod pod as a new Kubernetes service on master.

```bash
kubectl expose pod sevim-pod --port=80 --type=NodePort
```
...
- Check the browser (`http://<public-workerNode-ip>:<node-port>`) that sevim-pod is running.

- Delete the `Pod`, the `PersistentVolumeClaim` and the `PersistentVolume`.

```bash
kubectl delete pod sevim-pod
kubectl delete pvc sevim-pv-claim
kubectl delete pv sevim-pv-vol
```
